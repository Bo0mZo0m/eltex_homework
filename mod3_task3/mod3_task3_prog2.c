...
int main()
{
    ...
    if(new){
        ...
        array_time[3] = (int)time_start; //Время "старта" второй программы
        array_time[6] = 2; // Флаг для переключения между программами!
        ...
        time_end = time(NULL);
        time_result = time_end - array_time[2];
        time_start = time(NULL);
        array_time[1] -=(int)time_result; // Общее время хода для второй прогрммы
        array_time[3] = (int)time_start; //Время "старта" второй программы
        array_time[5] = (int)time_result;//Время "хода" первой программы
        array_time[6] = 2; // Флаг для переключения между программами!
        printf("Общее время черных %d\n Время хода черных %d\n", array_time[1], array_time[5]); 
    }  
